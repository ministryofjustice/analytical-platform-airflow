...
data_providers = ["ho", "cps", "moj", "hmcts"]
...
def _send_email(data_provider):
    ...
    returned = context.get("task_instance").xcom_pull(
        task_ids=f"r-validation-{data_provider}"
    )
    max_timestamp = returned["max_timestamp"]
    result = returned["overall_result"]
    task_id = f"send-email-{data_provider}"
    email_addresses = ["joe.dods@justice.gov.uk", "rolake.odebunmi@justice.gov.uk"]
    subject = f"New data validation output ready - {data_provider.upper()} - {result}"
    # HTML contents of email
    email_contents = (
        "<h3>A new data validation output has been created</h3>"
...
for data_provider in data_providers:
    ...
    task_2 = PythonOperator(
        dag=dag, task_id=task_id_2, python_callable=email_func, provide_context=True
    )
